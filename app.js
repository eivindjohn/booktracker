...
function syncUserToFirebase() {
    ...
    // Create simplified book list for sharing
    const sharedBooks = userBooks.map(b => ({
        title: b.title,
...
const BookAPI = {
    ...
    async search(query) {
        ...
        return (data.docs || []).map(doc => ({
            ...
            workKey: doc.key || '' // Save the work key for page count lookup
        }));
    },
    ...
    async getPageCount(book) {
        console.log('Getting page count for:', book.title);
        
        // If we already have page count, return it
        if (book.pageCount && book.pageCount > 0) {
            console.log('Already has page count:', book.pageCount);
        ...
        if (book.isbn) {
            try {
                ...
                console.log('Trying ISBN lookup:', cleanISBN);
                const response = await fetch(`${this.baseURL}/isbn/${cleanISBN}.json`);
                if (response.ok) {
                    const data = await response.json();
                    console.log('ISBN data:', data);
                    if (data.number_of_pages && data.number_of_pages > 0) {
                        console.log('Found pages via ISBN:', data.number_of_pages);
        ...
        // Try works API if we have a key
        if (book.workKey) {
            try {
                console.log('Trying works API:', book.workKey);
                const response = await fetch(`${this.baseURL}${book.workKey}/editions.json?limit=10`);
                if (response.ok) {
                    const data = await response.json();
                    if (data.entries) {
                        for (const edition of data.entries) {
                            if (edition.number_of_pages && edition.number_of_pages > 0) {
                                console.log('Found pages via editions:', edition.number_of_pages);
                ...
                console.log('Works API failed:', e);
            }
        }
        
        // Try direct search for this specific book with author
        try {
            const authorQuery = book.authors?.[0] ? ` ${book.authors[0]}` : '';
            const query = encodeURIComponent(book.title + authorQuery);
            console.log('Trying search:', book.title + authorQuery);
            const response = await fetch(`${this.baseURL}/search.json?q=${query}&limit=5`);
            const data = await response.json();
            
            if (data.docs && data.docs.length > 0) {
                // Find best match with page count
                for (const doc of data.docs) {
                    // Check median pages
                    if (doc.number_of_pages_median && doc.number_of_pages_median > 0) {
                        console.log('Found pages via search (median):', doc.number_of_pages_median);
                        return doc.number_of_pages_median;
                    }
                }
                
                // Try first result's ISBN
                const firstDoc = data.docs[0];
                if (firstDoc.isbn && firstDoc.isbn.length > 0) {
                    for (const isbn of firstDoc.isbn.slice(0, 3)) {
                        try {
                            console.log('Trying ISBN from search:', isbn);
                            const isbnResponse = await fetch(`${this.baseURL}/isbn/${isbn}.json`);
                            if (isbnResponse.ok) {
                                const isbnData = await isbnResponse.json();
                                if (isbnData.number_of_pages && isbnData.number_of_pages > 0) {
                                    console.log('Found pages via ISBN lookup:', isbnData.number_of_pages);
        ...
        const googlePages = await this.getPageCountFromGoogle(book);
        if (googlePages > 0) {
            return googlePages;
        }
        
        console.log('Could not find page count for:', book.title);
        return 0;
    },
    
    // Google Books API fallback
    async getPageCountFromGoogle(book) {
        try {
            // Build search query
            let query = `intitle:${encodeURIComponent(book.title)}`;
            if (book.authors?.[0]) {
                query += `+inauthor:${encodeURIComponent(book.authors[0])}`;
            }
            
            // Try ISBN first if available
            if (book.isbn) {
                const cleanISBN = book.isbn.replace(/-/g, '').trim();
                console.log('Google Books: trying ISBN:', cleanISBN);
                const isbnResponse = await fetch(`${this.googleBooksURL}?q=isbn:${cleanISBN}`);
                if (isbnResponse.ok) {
                    const isbnData = await isbnResponse.json();
                    if (isbnData.items?.[0]?.volumeInfo?.pageCount) {
                        console.log('Found pages via Google Books ISBN:', isbnData.items[0].volumeInfo.pageCount);
                        return isbnData.items[0].volumeInfo.pageCount;
                    }
                }
            }
            
            // Try title/author search
            console.log('Google Books: trying search:', query);
            const response = await fetch(`${this.googleBooksURL}?q=${query}&maxResults=5`);
            if (response.ok) {
                const data = await response.json();
                if (data.items && data.items.length > 0) {
                    ...
                    for (const item of data.items) {
                        ...
                        if (pageCount && pageCount > 0) {
                            // Verify it's the right book by checking title similarity
                            const resultTitle = item.volumeInfo?.title?.toLowerCase() || '';
                            const searchTitle = book.title.toLowerCase();
                            if (resultTitle.includes(searchTitle) || searchTitle.includes(resultTitle)) {
                                console.log('Found pages via Google Books:', pageCount);
                                return pageCount;
                            }
                        }
                    }
                    
                    // If no exact match, return first result with page count
                    for (const item of data.items) {
                        const pageCount = item.volumeInfo?.pageCount;
                        if (pageCount && pageCount > 0) {
                            console.log('Found pages via Google Books (first result):', pageCount);
                            return pageCount;
                        }
                    }
                }
            ...
            console.log('Google Books API failed:', e);
        }
        return 0;
    }
};
...
function showPage(pageName) {
    ...
    // Stop camera when leaving add page
    if (pageName !== 'add' && scannerRunning) {
        stopCamera();
    }
}
...
// Show a user's books in a modal
function showUserBooks(userIndex) {
    const entries = window.leaderboardEntries;
    if (!entries || !entries[userIndex]) return;
    ...
    let booksHtml = '';
    if (books.length === 0) {
        booksHtml = '<p style="text-align: center; color: var(--text-secondary); padding: 2rem 0;">No books yet</p>';
    } else {
        // Group by status
        ...
        if (reading.length > 0) {
            booksHtml += `<h4 style="margin: 1rem 0 0.5rem; color: var(--primary);">📖 Currently Reading</h4>`;
            booksHtml += reading.map(b => createUserBookItem(b)).join('');
        }
        
        if (completed.length > 0) {
            booksHtml += `<h4 style="margin: 1rem 0 0.5rem; color: var(--success);">✓ Completed</h4>`;
            booksHtml += completed.map(b => createUserBookItem(b)).join('');
        }
        
        if (wantToRead.length > 0) {
            booksHtml += `<h4 style="margin: 1rem 0 0.5rem; color: var(--warning);">📚 Want to Read</h4>`;
            booksHtml += wantToRead.map(b => createUserBookItem(b)).join('');
        }
    }
    
    modal.innerHTML = `
        <div class="modal-header">
            <h3>${escapeHtml(user.name)}'s Books</h3>
            <button class="modal-close" onclick="hideModal()">×</button>
        </div>
        <div style="display: flex; justify-content: center; gap: 2rem; margin-bottom: 1rem;">
            <div style="text-align: center;">
                <div style="font-size: 1.5rem; font-weight: bold; color: var(--primary);">${user.pagesRead}</div>
                <div style="font-size: 0.75rem; color: var(--text-secondary);">pages read</div>
            </div>
            <div style="text-align: center;">
                <div style="font-size: 1.5rem; font-weight: bold; color: var(--primary);">${user.booksCompleted}</div>
...
function createUserBookItem(book) {
    const coverImg = book.coverURL 
        ? `<img src="${book.coverURL}" alt="" style="width: 40px; height: 60px; object-fit: cover; border-radius: 4px;">`
        : `<div style="width: 40px; height: 60px; background: var(--secondary); border-radius: 4px; display: flex; align-items: center; justify-content: center;">📖</div>`;
    
    const progressBar = book.status === 'reading' ? `
        <div style="height: 4px; background: var(--border); border-radius: 2px; margin-top: 4px;">
            <div style="height: 100%; width: ${book.percentComplete}%; background: var(--primary); border-radius: 2px;"></div>
        </div>
    ` : '';
    ...
    return `
        <div style="display: flex; gap: 0.75rem; padding: 0.5rem 0; border-bottom: 1px solid var(--border);">
            ...
            <div style="flex: 1; min-width: 0;">
                <div style="font-weight: 500; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">${escapeHtml(book.title)}</div>
                <div style="font-size: 0.75rem; color: var(--text-secondary);">${escapeHtml(book.authors?.join(', ') || 'Unknown')}</div>
                ${book.status === 'reading' ? `<div style="font-size: 0.625rem; color: var(--primary);">${book.percentComplete}% complete</div>` : ''}
                ${progressBar}
            </div>
        </div>
    `;
